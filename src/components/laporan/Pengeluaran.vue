...
export default {
  ...
  computed: {
    ...
    totalPengeluaran() {
      return this.pengeluaran.reduce(function(prev, cur) {
    ...
    getData(month) {
      ...
      db.doc(`users/${this.user.uid}`)
        ...
        .onSnapshot((snapshot) => {
          ...
          this.$store.commit("setPengeluaran", this.totalPengeluaran);
          this.$store.commit("setDataPengeluaran", this.pengeluaran);
        });
    },
    rupiahFormat(data) {
